...
class GA:
    ...
    def perform(self, data: Any, nro_chromosomes: int = 10, offspring_count=2, iterations=1000):
        self._population.init(data, nro_chromosomes)
        print(f"Original Population: {self._population.get_pop()}")
        i = 0
        bests = []
        equal_count = iterations * 0.4
        while i < iterations:
            ...
            print(f"Mutated Offspring: {offspring}")
            self._population.add_offspring(offspring)
            print(f"Dataset:{self._population.get_pop()}")
            ...
            if len(bests) > 5:
                if bests[-2][1] == best_distance[1]:
                    ...
                    if equal_count == 0:
                        break
            i += 1
        print(f"Summary: {bests}")
        print(f"Best Path:{self.get_pop()[bests[-1][0]]}")
